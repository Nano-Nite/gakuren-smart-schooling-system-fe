...
import * as detailUtils from "../src/features/students/utils/studentDetail.js";
import * as payloadUtils from "../src/features/students/utils/studentUpdatePayload.js";
import * as classUtils from "../src/features/students/utils/resolveStudentClass.js";
import * as genderUtils from "../src/features/students/utils/resolveStudentGender.js";
...
test("activation resolves IDs from the backend detail response containing only class and gender names", async () => {
  ...
  const imports = {
    "../../../shared/config/api": { default: { GET_STUDENTS: "/student/get", GET_CLASSES: "/class/get" } },
    "../../../shared/utils/api": { authenticatedRequest: request },
    ...
    "../../../shared/utils/dailyReferenceCache": { getDailyReference: async type => { assert.equal(type, "gender"); return { result: [{ UUID: "gender-1", Name: "Laki-laki" }] }; } },
  };
  const context = vm.createContext({});
  const module = new vm.SourceTextModule(await readFile("src/features/students/utils/studentActivation.js", "utf8"), { context });
