...
export const attendanceService = {
  async getLocations(signal) {
    const response = await authenticatedRequest(
      "/v1/attendance/location/get-all",
      { method: "GET", signal },
    );
  ...
  async closeSession(sessionUuid) {
    return unwrap(
      await authenticatedRequest(
        `/v1/attendance/sessions/${encodeURIComponent(sessionUuid)}/close`,
        { method: "POST" },
      ),
    );
  },
  ...
  async getSessionAttendances(sessionUuid, signal) {
    const response = await authenticatedRequest(
      `/v1/attendance/sessions/${encodeURIComponent(sessionUuid)}/attendances`,
      { signal },
    );
    const data = unwrap(response);
    return {
  ...
  async scanQr(payload) {
    return unwrap(
      await authenticatedRequest("/v1/attendance/scan", {
        method: "POST",
        body: payload,
      }),
    );
  ...
  async getIdentityCredential(signal) {
    return unwrap(
      await authenticatedRequest("/v1/attendance/identity-credential", {
        signal,
      }),
    );
  },
  ...
  async getOfflineConfig(signal) {
    return unwrap(
      await authenticatedRequest("/v1/attendance/offline-config", { signal }),
    );
  },
  ...
  async syncOfflineAttendances(deviceUuid, records) {
    return unwrap(
      await authenticatedRequest("/v1/attendance/offline-sync", {
        method: "POST",
        body: { device_uuid: deviceUuid, records },
      }),
    );
  },
};
