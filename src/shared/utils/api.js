...
import { clearOfflineSessionCache } from "../../features/attendance/services/offlineAttendanceStore";
import { signTrustedDeviceRequest, trustedDeviceHeaders } from "../../features/settings/device/services/trustedDeviceSigner";
...
export const authenticatedRequest = async (
  ...
  if (options.trustedDevice) {
    ...
    Object.assign(config.headers, trustedDeviceHeaders(proof));
    if (requestVersion !== sessionVersion || requestToken !== getAccessToken()) throw new Error('Sesi telah berubah. Coba lagi.');
  }
  ...
  try {
    ...
    if (response.status === 401) {
      if (!retried) {
        ...
        if (requestVersion !== sessionVersion)
          throw new Error("Sesi telah berubah.");
        return authenticatedRequest(endpoint, { ...options, body: config.body }, true);
      }
      await clearAuthData();
      throw Object.assign(new Error(data.message || ERROR_MESSAGES.UNAUTHORIZED), { status: 401 });
    }
    if (!response.ok || data.error) {
