...
import { getUserData, isNetworkAvailable } from "../../../shared/utils/api";
...
export default function MyIdentityQr() {
  ...
  return <div className="w-full max-w-md text-center"><div className="mx-auto inline-flex items-center gap-2 rounded-full bg-blue-50 px-3 py-1.5 text-xs font-semibold text-blue-700"><QrCode className="h-4 w-4" />QR Identitas Saya</div><h2 className="mt-4 text-lg font-bold">Tunjukkan QR ini kepada petugas</h2><p className="mt-1 text-sm text-slate-500">QR tersimpan aman di perangkat dan tetap tersedia tanpa internet.</p><div className="mx-auto mt-4 w-full max-w-[280px] rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><QRCodeSVG value={credential.qr_token} size={260} className="h-auto w-full" level="M" marginSize={1} /></div><p className="mt-4 text-xs text-slate-400">Kredensial {credential.credential_id || "terverifikasi"} · versi {credential.version ?? credential.credential_version ?? 1}</p></div>;
}
