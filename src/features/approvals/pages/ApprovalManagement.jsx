...
import ExpandableBadges from "../../../shared/components/ExpandableBadges";
...
import { authenticatedRequest, getUserData } from "../../../shared/utils/api";
...
const approvalFieldSchemas = {
  location: [["code", "Kode Lokasi"], ["name", "Nama Lokasi"], ["latitude", "Lintang (Latitude)"], ["longitude", "Bujur (Longitude)"], ["radius_meter", "Radius (meter)"], ["geofence_radius_meter", "Radius (meter)"]],
  ...
  async function openApprovalDetail(row) {
    ...
    try {
      ...
      if (["UPDATE", "ACTIVATE"].includes(String(nextDetail.action).toUpperCase()) && nextDetail.entityUuid) {
        ...
        try {
          const activeResponse = endpoint === API_CONFIG.GET_TEACHER_STAFF
            ? await authenticatedRequest(`${endpoint}?uuid=${encodeURIComponent(nextDetail.entityUuid)}`, {
              method: "GET",
              signal: controller.signal,
            })
  ...
  return <>
    ...
    <div className="p-4 sm:p-6">
      ...
      <section className="approval-table data-table-card overflow-hidden rounded-xl border border-slate-200 bg-white shadow-card">
        <div aria-label="Menu persetujuan" className="mx-4 mt-4 grid grid-cols-4 gap-1 rounded-xl bg-slate-100 p-1 dark:bg-slate-800 sm:mx-5 sm:mt-5">
          {[["mine", "Pengajuan Saya"], ["waiting", "Perlu Tindakan"], ["processed", "Riwayat Keputusan"], ["all", "Semua Pengajuan"]].map(([value, label]) => <button key={value} type="button" aria-pressed={activeTab === value} onClick={() => { setActiveTab(value); setPage(1); }} className={`flex min-h-11 min-w-0 items-center justify-center rounded-lg px-2 py-2 text-xs font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 sm:text-sm ${activeTab === value ? "bg-white text-blue-700 shadow-sm dark:bg-slate-700 dark:text-blue-300" : "text-slate-500 hover:text-blue-600 dark:text-slate-400"}`}>{label}</button>)}
        </div>
...
function getApprovalEntityEndpoint(entityType, moduleName, requestData) {
  if (isTeacherStaffEntity(entityType)) return API_CONFIG.GET_TEACHER_STAFF;
...
function ApprovalValue({ value, fieldKey, entityType }) {
  ...
  const isTeacherStaff = isTeacherStaffEntity(entityType);
...
function ActivationApprovalDetails({ approval, onRetry }) {
  ...
  const isLocation = getApprovalFieldSchema(approval.entityType, request) === approvalFieldSchemas.location;
  return <div className="space-y-4">
    ...
    {isLocation && <ApprovalUpdateDetails canCompare={false} rows={getApprovalRequestEntries({ ...current, ...request }, approval.entityType).filter(([key]) => !["name", "status"].includes(normalizeApprovalFieldKey(key))).map(([key, value, label]) => ({ key, value, label }))} renderValue={(value, key) => <ApprovalValue value={sanitizeRequestValue(value)} fieldKey={key} entityType={approval.entityType} />} />}
...
function ApprovalDetailDrawer({ approval, currentUser, loading, error, actionError, actionProgress, executing, note, setNote, canApprove, canReject, onClose, onDecide, onCancel, onRetry }) {
  ...
  const isLocation = getApprovalFieldSchema(approval.entityType, approval.requestData || {}) === approvalFieldSchemas.location;
  ...
  return <div className="fixed inset-0 z-[70] flex justify-end" role="dialog" aria-modal="true" aria-label="Detail pengajuan">
    ...
    <aside className="relative flex h-full w-full max-w-xl flex-col bg-white shadow-2xl">
      ...
      <div className={`min-h-0 flex-1 space-y-5 overflow-y-auto p-5 ${!loading && !error && isActive ? "pb-32 scroll-pb-32" : ""}`}>
...
function ProgressStep({ step, index, stepNumber, isLast, cancelIndex, approvalStatus }) {
  ...
  const nodeClass = isSubmission ? "border-2 border-slate-300 bg-transparent dark:border-slate-600" : isNegativeDecision ? "bg-rose-600 text-white" : isAffectedByCancellation ? "approval-timeline-node-cutout border-2 border-rose-400 bg-rose-50 text-rose-600" : state === "past" ? "bg-emerald-600 text-white" : state === "current" ? "bg-amber-500 text-white" : "approval-timeline-node-cutout border-2 border-slate-300 bg-white text-slate-500";
  const nodeSizeClass = shouldPulse ? "-ml-1 h-9 w-9 text-sm" : "h-7 w-7 text-xs";
  const lineClass = isSubmission ? "bg-slate-300 dark:bg-slate-600" : isNegativeDecision || (cancelIndex >= 0 && index >= cancelIndex) ? "bg-rose-400" : state === "past" ? "bg-emerald-400" : "bg-slate-200";
  const titleClass = isSubmission ? "text-slate-900 dark:text-slate-100" : isNegativeDecision || isAffectedByCancellation ? "text-rose-600" : state === "current" ? "text-amber-600" : "";
...
function getApprovalFieldSchema(entityType, requestData) {
  if (isTeacherStaffEntity(entityType)) return approvalFieldSchemas.teacher;
  ...
  if (/location|lokasi|(^|[^a-z])loc([^a-z]|$)/.test(normalizedType) || (normalizedKeys.has("latitude") && normalizedKeys.has("longitude"))) return approvalFieldSchemas.location;
