...
export default function TitleSelector({ prefixValues = [], suffixValues = [], onChange, previewName = "" }) {
  ...
  const renderGroup = (type, label, values) => {
    ...
    return <div className="relative"><span className="mb-2 block text-sm font-semibold">{label} <span className="font-normal text-slate-400">(opsional)</span></span><button ref={element => { triggerRefs.current[type] = element; }} type="button" aria-expanded={open === type} aria-label={`Pilih ${label.toLowerCase()}`} onClick={() => { setSearch(""); setOpen(current => current === type ? null : type); }} title={selected.map(option => option.label).join(", ")} className="flex h-10 w-full items-center justify-between gap-2 rounded-lg border border-slate-300 bg-white px-3 text-left text-sm hover:border-blue-300"><span className="flex min-w-0 flex-1 items-center gap-1.5">{selected.length ? <><span className="truncate rounded-md bg-blue-50 px-2 py-1 text-xs font-semibold text-blue-700">{selected[0].label}</span>{selected.length > 1 && <span className="shrink-0 rounded-md bg-slate-100 px-2 py-1 text-xs font-medium text-slate-600" aria-label={`${selected.length - 1} gelar lainnya dipilih`}>+{selected.length - 1}</span>}</> : <span data-placeholder="true" className="truncate text-slate-400">Pilih {label.toLowerCase()}</span>}</span><ChevronDown className="h-4 w-4 shrink-0 text-slate-500" /></button>{menu}</div>;
  };
