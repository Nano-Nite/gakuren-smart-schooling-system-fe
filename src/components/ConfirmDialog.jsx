...
export default function ConfirmDialog({ open, title = "Konfirmasi tindakan", description, information = [], consequences = [], confirmLabel = "Hapus", cancelLabel = "Batal", tone = "danger", error, submitting = false, onConfirm, onCancel }) {
  const titleId = useId();
  const descriptionId = useId();
  const warningHintId = useId();
  const [warningReviewed, setWarningReviewed] = useState(false);
  useEffect(() => { setWarningReviewed(false); }, [open]);
  ...
  const warningLocked = danger && consequences.length > 0 && !warningReviewed;
  ...
  return <div className="fixed inset-0 z-[60] grid place-items-center p-4">
    ...
    <section role={danger ? "alertdialog" : "dialog"} aria-modal="true" aria-labelledby={titleId} aria-describedby={descriptionId} className="confirm-dialog relative max-h-[calc(100dvh-2rem)] w-full max-w-md overflow-y-auto rounded-2xl border border-slate-200 bg-white shadow-2xl dark:border-white/10 dark:bg-[var(--mui-paper)]">
      ...
      <div className="px-6 pb-5 pt-6 text-left sm:px-7 sm:pt-7">
        ...
        {information.length > 0 && <div role="note" className="mt-4 rounded-xl border border-blue-200 bg-blue-50/60 p-3.5 dark:border-blue-400/20 dark:bg-blue-400/5">
          <div className="flex items-center gap-2 text-blue-700 dark:text-blue-300"><Info aria-hidden="true" className="h-4 w-4 shrink-0" /><h3 className="text-xs font-semibold">Setelah data diaktifkan</h3></div>
          <ul className="mt-2 list-disc space-y-2 pl-5 text-xs leading-5 text-slate-600 dark:text-slate-300">{information.map(text => <li key={text}>{text}</li>)}</ul>
        </div>}
        {error && <p role="alert" className="mt-3 text-sm leading-6 text-rose-600 dark:text-rose-400">{error}</p>}
        {consequences.length > 0 && <details onToggle={event => { if (event.currentTarget.open) setWarningReviewed(true); }} className="group mt-4 rounded-xl border border-slate-200 dark:border-white/10">
        ...
        {warningLocked && <p id={warningHintId} className="mt-3 text-xs leading-5 text-amber-700 dark:text-amber-300">Buka rincian peringatan di atas untuk mengaktifkan tombol konfirmasi.</p>}
      </div>
      <footer className="grid grid-cols-2 gap-3 px-6 pb-6 pt-1 sm:px-7 sm:pb-7">
        ...
        <button disabled={submitting || warningLocked} aria-describedby={warningLocked ? warningHintId : undefined} type="button" onClick={() => { if (!submitting && !warningLocked) onConfirm(); }} className={`action-lift min-h-11 rounded-xl px-4 py-2.5 text-sm font-semibold text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-40 dark:focus-visible:ring-offset-slate-900 ${buttonClasses}`}>{confirmLabel}</button>
      </footer>
    </section>
  </div>;
}
