...
export default function AttendanceSessionForm({ loading, allowed, error, onSubmit }) {
  ...
  return <form onSubmit={submit} className="space-y-5 p-1">
    ...
    <div className="grid gap-4 sm:grid-cols-2">
      ...
      <label className="text-sm font-semibold">Berlaku Mulai<input required type="datetime-local" value={form.valid_from} onChange={event => update("valid_from", event.target.value)} className={fieldClass} /></label>
      <label className="text-sm font-semibold">Berlaku Sampai<input required type="datetime-local" min={form.valid_from} value={form.valid_until} onChange={event => update("valid_until", event.target.value)} className={fieldClass} /></label>
      <label className="text-sm font-semibold">Target<Select className="mt-2" value={form.target_type} onChange={value => update("target_type", value)} ariaLabel="Target absensi" options={[{ value: "ALL", label: "Semua" }, { value: "STUDENT", label: "Siswa" }, { value: "STAFF", label: "Guru / Pegawai" }]} /></label>
