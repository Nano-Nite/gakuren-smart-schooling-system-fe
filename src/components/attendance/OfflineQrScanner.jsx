...
const statusLabel = { PENDING_SYNC: "Pending Sync", VERIFIED: "Verified", FLAGGED: "Flagged", REJECTED: "Rejected" };
...
export default function OfflineQrScanner() {
  ...
  const processToken = async rawValue => {
    ...
    try {
      ...
      setError(message || "Credential tidak dapat diverifikasi secara offline.");
      setResult(null);
    } finally {
      setProcessing(false);
  ...
  return <div className="w-full space-y-5">
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1.3fr)_minmax(300px,.7fr)]">
      <section className="rounded-2xl border border-slate-200 p-5">
        <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="flex items-center gap-2 text-xl font-bold"><ScanLine className="h-5 w-5 text-amber-500" />Scan QR Pengguna</h2><p className="mt-1 text-sm text-slate-500">Arahkan QR siswa atau pegawai ke kamera.</p></div><div className="text-right"><span className="inline-flex items-center gap-1.5 rounded-full bg-amber-50 px-3 py-1.5 text-xs font-bold text-amber-700"><WifiOff className="h-3.5 w-3.5" />Mode Offline</span>{context?.device && <p className="mt-2 text-xs font-semibold text-slate-600">{context.device.device_name || "Perangkat absensi"}</p>}</div></div>
        <p className="mt-4 rounded-xl bg-slate-50 px-4 py-3 text-sm text-slate-600">Data kehadiran disimpan di perangkat ini dan disinkronkan ketika backend kembali tersedia.</p>
      ...
      <aside className="space-y-4"><section className="rounded-2xl border border-slate-200 p-4"><div className="grid grid-cols-2 gap-3"><div className="rounded-xl bg-amber-50 p-3"><p className="text-xs text-amber-700">Pending Sync</p><p className="mt-1 text-2xl font-bold text-amber-800">{attendance.pendingCount}</p></div><div className="rounded-xl bg-blue-50 p-3"><p className="text-xs text-blue-700">Scan Hari Ini</p><p className="mt-1 text-2xl font-bold text-blue-800">{attendance.todayCount}</p></div></div><button type="button" disabled={!attendance.online || !attendance.pendingCount || attendance.syncState === "SYNCING"} onClick={attendance.syncNow} title={!attendance.online ? "Internet diperlukan untuk sinkronisasi." : undefined} className="mt-3 flex w-full items-center justify-center gap-2 rounded-lg bg-blue-600 py-2.5 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-40"><RefreshCw className={`h-4 w-4 ${attendance.syncState === "SYNCING" ? "animate-spin" : ""}`} />{attendance.syncState === "SYNCING" ? `Menyinkronkan ${attendance.pendingCount} data…` : "Sinkronkan Sekarang"}</button>{!attendance.online && <p className="mt-2 text-center text-xs text-slate-500">Internet diperlukan untuk sinkronisasi.</p>}{attendance.online && !attendance.pendingCount && <p className="mt-2 text-center text-xs font-semibold text-emerald-600">Semua data telah tersinkronisasi.</p>}{attendance.reviewCount > 0 && <p className="mt-2 text-center text-xs font-semibold text-orange-600">{attendance.reviewCount} data perlu diperiksa.</p>}{attendance.syncError && <p className="mt-2 text-xs text-rose-600">{attendance.syncError}</p>}</section>
        <section className="overflow-hidden rounded-2xl border border-slate-200"><header className="border-b border-slate-200 px-4 py-3"><h3 className="font-bold">Scan Terbaru</h3></header><div className="max-h-[420px] divide-y divide-slate-100 overflow-y-auto">{attendance.recent.length ? attendance.recent.map(item => <div key={item.local_uuid || item.local_id} className="p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-sm font-semibold">{item.display_name || "User Gakuren"}</p><p className="mt-0.5 truncate text-xs text-slate-500">{item.person_type || "User"}{item.class_name ? ` • ${item.class_name}` : ""}</p><p className="mt-1 flex items-center gap-1 text-xs text-slate-400"><Clock3 className="h-3 w-3" />{new Date(item.recorded_at || item.created_at).toLocaleTimeString("id-ID", { hour: "2-digit", minute: "2-digit" })}</p></div><span className={`shrink-0 rounded-full px-2 py-1 text-[10px] font-bold ${statusStyle[item.sync_status] || "bg-slate-100 text-slate-600"}`}>{statusLabel[item.sync_status] || item.sync_status}</span></div></div>) : <div className="p-8 text-center text-sm text-slate-500"><ShieldCheck className="mx-auto mb-2 h-8 w-8 text-slate-300" />Belum ada scan offline.</div>}</div></section></aside>
    </div>
  </div>;
}
