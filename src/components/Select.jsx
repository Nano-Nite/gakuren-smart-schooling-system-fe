...
export default function Select({ value, options, onChange, ariaLabel, placement = "bottom", className = "" }) {
  ...
  return <div ref={rootRef} className={`relative ${className}`}>
    <button ref={buttonRef} type="button" aria-label={ariaLabel} aria-haspopup="listbox" aria-expanded={open} onClick={() => setOpen(state => !state)} onKeyDown={handleKeyDown} className="flex h-10 w-full items-center justify-between gap-3 rounded-lg border border-slate-200 bg-white px-3 text-left text-sm text-slate-700 shadow-sm hover:border-blue-300 focus-visible:border-blue-500"><span className="whitespace-nowrap">{selected?.label}</span><ChevronDown className={`h-4 w-4 shrink-0 text-slate-500 transition-transform duration-200 ${open ? "rotate-180" : ""}`} /></button>
