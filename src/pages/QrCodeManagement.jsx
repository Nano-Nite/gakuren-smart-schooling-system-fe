...
import { Camera, CameraOff, CheckCircle2, RefreshCw, QrCode, ScanLine, UserRound } from "lucide-react";
...
function CameraScanner() {
  ...
  return <div className="w-full max-w-lg text-center"><h2 className="text-lg font-bold">Arahkan kamera ke QR Code</h2><p className="mt-1 text-sm text-slate-500">Lokasi akan diminta setelah QR berhasil dibaca.</p><div className="relative mx-auto mt-4 grid aspect-square w-full max-w-[300px] place-items-center overflow-hidden rounded-3xl border-2 border-dashed border-blue-300 bg-slate-900"><video ref={videoRef} playsInline muted className={`h-full w-full object-cover ${active ? "block" : "hidden"}`} />{!active && !result && !busy && <Camera className="h-12 w-12 text-slate-500" />}{active && <><span className="pointer-events-none absolute inset-8 rounded-2xl border-2 border-blue-400" /><span className="pointer-events-none absolute inset-x-10 top-1/2 h-0.5 animate-pulse bg-blue-400" /></>}{(busy || result) && <div className="p-6 text-white"><CheckCircle2 className={`mx-auto h-16 w-16 ${result ? "text-emerald-400" : "animate-pulse text-blue-400"}`} /><p className="mt-3 font-semibold">{busy ? "Memvalidasi kehadiran…" : result?.message || "Kehadiran berhasil tercatat"}</p>{result?.distance_meter != null && <p className="mt-1 text-xs text-slate-300">Jarak {result.distance_meter} meter</p>}</div>}</div><div className="mx-auto mt-4 max-w-[300px] text-left"><div className="flex items-end gap-3"><div className="min-w-0 flex-1"><p className="mb-1.5 text-xs font-medium text-slate-500">Sumber kamera</p><Select value={selectedCamera} onChange={value => { setSelectedCamera(value); if (active) startCamera(value); }} options={[{ value: "", label: "Kamera otomatis" }, ...cameras]} ariaLabel="Pilih sumber kamera" placement="top" /></div>{active ? <button onClick={stopCamera} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-lg border border-rose-200 px-3 text-sm font-semibold text-rose-600" aria-label="Tutup kamera"><CameraOff className="h-4 w-4" />Stop</button> : <button disabled={busy} onClick={() => startCamera()} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-lg bg-blue-600 px-3 text-sm font-semibold text-white hover:bg-blue-700 disabled:opacity-50" aria-label="Aktifkan kamera"><Camera className="h-4 w-4" />Mulai</button>}</div>{error && <p className="mt-3 rounded-xl border border-rose-200 bg-rose-50 p-3 text-xs text-rose-700">{error}</p>}{(error || result) && <button type="button" onClick={() => { startCamera(); }} className="mt-3 w-full rounded-lg border border-slate-200 py-2 text-xs font-semibold text-slate-600">Pindai lagi</button>}</div></div>;
}
...
export default function QrCodeManagement() {
  ...
  return <><Helmet><title>QR Code | Gakuren</title></Helmet><div className="qr-page mx-auto max-w-[1280px] p-4 sm:p-6"><OfflineSyncStatus /><section className={`qr-dashboard-shell ${tab === "create" ? "lg:grid" : "max-w-2xl mx-auto"} lg:grid-cols-[minmax(0,1fr)_300px] lg:grid-rows-[auto_auto] lg:gap-x-4`}><div className="qr-workspace-surface min-w-0 rounded-t-2xl border border-b-0 border-slate-200 bg-white px-4 pt-4 lg:col-start-1 lg:row-start-1 lg:px-5"><QrNavigation tab={tab} onChange={setTab} online /></div>{tab === "create" ? <CreateWorkspace /> : <div className="qr-workspace-surface grid place-items-center rounded-b-2xl border border-t-0 border-slate-200 bg-white p-5 lg:col-start-1 lg:row-start-2">{tab === "identity" ? <MyIdentityQr /> : <CameraScanner />}</div>}</section></div></>;
}
