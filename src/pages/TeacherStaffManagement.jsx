...
export default function TeacherStaffManagement() {
  ...
  const saveCreate = event => { event.preventDefault(); const errors = Object.fromEntries(textFields.map(([, key]) => [key, validateTeacherField(key, form[key])]).filter(([, message]) => message)); setFormErrors(errors); if (Object.keys(errors).length) return; setRows(current => [{ ...form, id: `local-${Date.now()}` }, ...current]); setStatistics(current => ({ ...current, total_row: current.total_row + 1, end_row: current.end_row + 1 })); setCreating(false); };
