...
import { ArrowDown, ArrowDownUp, ArrowLeft, ArrowUp, Check, Download, Pencil, Plus, RefreshCw, Search, Trash2, Upload, X } from "lucide-react";
...
const createEmptyForm = () => ({ name: "", nik: "", nuptk: "", nip: "", email: "", phone: "", position: "", position_uuids: [], status: "Aktif", gender_uuid: "", gender: "", birth_place: "", birth_date: "", address: "", educations: [newEducation(true)], role: "", subject: "", employment_status: "", employment_status_uuid: "", title_prefix_uuids: [], title_suffix_uuids: [], title_prefixes: [], title_suffixes: [] });
...
const wizardFields = {
  ...
  2: ["educations"],
  3: ["role"],
  4: ["position_uuids", "employment_status_uuid"],
};
...
export default function TeacherStaffManagement() {
  ...
  const validateCreateStep = step => {
    ...
    const fields = [...wizardFields[step], ...(step === 4 ? (form.role === "teacher" ? ["nik", "nuptk", "nip", "subject"] : []) : [])];
  ...
  const addCreatedRow = () => {
    ...
    const created = { ...form, position: form.position.trim(), status: "Aktif", roleLabel, id: `local-${Date.now()}` };
  ...
  const saveCreate = event => {
    ...
    const allErrors = {};
    [1, 3, 4].forEach(step => [...wizardFields[step], ...(step === 4 ? (form.role === "teacher" ? ["nik", "nuptk", "nip", "subject"] : []) : [])].forEach(key => {
    ...
    if (Object.keys(allErrors).length) {
      const firstInvalidStep = Object.keys(allErrors).some(key => key.startsWith("education-")) ? 2 : [1, 3, 4].find(step => [...wizardFields[step], ...(step === 4 ? (form.role === "teacher" ? ["nik", "nuptk", "nip", "subject"] : []) : [])].some(key => allErrors[key]));
  ...
  const createWizard = <div className="employee-wizard min-w-0"><AnimatedStepper steps={wizardSteps} activeStep={createStep} disabled={transitioning} onStepChange={setCreateStep} />
    <StepTransition phase={phase} direction={direction} onAnimationEnd={onAnimationEnd}>
    <div className="mb-6 flex items-center justify-between gap-3"><div className="min-w-0"><p className="text-xs font-semibold uppercase tracking-wider text-blue-600">Langkah {displayedStep} dari 4</p><h3 ref={headingRef} tabIndex={-1} className="mt-1 text-xl font-bold outline-none">{wizardSteps[displayedStep - 1]}</h3></div>{displayedStep > 1 && <button type="button" disabled={transitioning} onClick={() => setCreateStep(step => step - 1)} aria-label="Kembali ke langkah sebelumnya" title="Kembali ke langkah sebelumnya" className="inline-flex shrink-0 items-center gap-2 rounded-lg px-2 py-2 text-sm font-semibold text-slate-600 hover:text-blue-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 dark:text-slate-300 dark:hover:text-blue-400"><ArrowLeft className="h-4 w-4" /><span>Kembali</span></button>}</div>
    ...
    {displayedStep === 4 && <div className="grid grid-cols-1 gap-5">{form.role === "teacher" && <>{wizardInput("NIK", "nik", { numeric: true, placeholder: "Masukkan NIK" })}{wizardInput("NUPTK", "nuptk", { optional: true, numeric: true, placeholder: "Masukkan NUPTK" })}{wizardInput("NIP", "nip", { optional: true, numeric: true, placeholder: "Masukkan NIP" })}</>}<PositionSelector key={form.role} isStaff={form.role === "staff"} values={form.position_uuids} error={formErrors.position_uuids} onChange={(uuids, names) => { setForm(current => ({ ...current, position_uuids: uuids, position: names.join(", ") })); if (formErrors.position_uuids) setFormErrors(current => ({ ...current, position_uuids: validateWizardField("position_uuids", uuids) })); }} />{form.role === "teacher" && wizardInput("Mata pelajaran", "subject", { optional: true, placeholder: "Contoh: Matematika" })}<EmployeeStatusSelect key={`employee-status-${form.role}`} isStaff={form.role === "staff"} value={form.employment_status_uuid} error={formErrors.employment_status_uuid} onChange={(uuid, name) => { setForm(current => ({ ...current, employment_status_uuid: uuid, employment_status: name })); if (formErrors.employment_status_uuid) setFormErrors(current => ({ ...current, employment_status_uuid: validateWizardField("employment_status_uuid", uuid) })); }} /></div>}
    </StepTransition>
  </div>;
  ...
  <FormDrawer open={creating} title="Tambah Guru/Staf" noValidate onClose={requestCreateClose} onSubmit={saveCreate} footerActions={<><button type="button" onClick={requestCreateClose} className="rounded-lg border border-slate-300 px-4 py-2.5 text-sm font-medium text-slate-700 hover:bg-slate-50">Batal</button><button type="submit" disabled={transitioning} className="rounded-lg bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white hover:bg-blue-700 disabled:cursor-wait disabled:opacity-60">{createStep === 4 ? "Simpan" : "Lanjut"}</button></>}>{createWizard}</FormDrawer>
