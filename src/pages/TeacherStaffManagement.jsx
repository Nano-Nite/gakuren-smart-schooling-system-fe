...
import { buildTeacherStaffPayload, getTeacherStaffCreateOutcome } from "../utils/teacherStaffPayload";
...
export default function TeacherStaffManagement() {
  ...
  useEffect(() => {
    ...
    const loadDetail = async () => {
      try {
        ...
        if (!response.data?.uuid) throw new Error("Respons detail guru/staf tidak valid.");
  ...
  const openDetail = item => { setDetailLoading(true); setDetailError(""); setFormErrors({}); setForm({ ...item }); setSelected(item); setEditing(false); };
  const openEdit = item => { openDetail(item); setEditing(true); };
  ...
  const saveEdit = event => { event.preventDefault(); if (!editing || detailLoading || detailError) return; const errors = Object.fromEntries(textFields.map(([, key]) => [key, validateTeacherField(key, form[key])]).filter(([, message]) => message)); setFormErrors(errors); if (Object.keys(errors).length) return; setRows(current => current.map(item => item.id === selected.id ? { ...item, ...form } : item)); setSelected(current => ({ ...current, ...form })); setEditing(false); };
  ...
  const createWizard = creating && <div className="employee-wizard min-w-0"><AnimatedStepper steps={wizardSteps} activeStep={createStep} disabled={transitioning || createSubmitting} onStepChange={setCreateStep} />
    ...
    {displayedStep === 4 && <div className="grid grid-cols-1 gap-5">{wizardInput("NIK", "nik", { numeric: true, placeholder: "Masukkan NIK" })}{form.role === "teacher" && wizardInput("NUPTK", "nuptk", { optional: true, numeric: true, placeholder: "Masukkan NUPTK" })}{wizardInput("NIP", "nip", { optional: true, numeric: true, placeholder: "Masukkan NIP" })}<PositionSelector key={form.role} isStaff={form.role === "staff"} values={form.position_uuids} error={formErrors.position_uuids} onChange={(uuids, names) => { setForm(current => ({ ...current, position_uuids: uuids, position: names.join(", ") })); if (formErrors.position_uuids) setFormErrors(current => ({ ...current, position_uuids: validateWizardField("position_uuids", uuids) })); }} />{form.role === "teacher" && <label className="block text-sm"><span className="mb-2 block font-semibold">Mata pelajaran <span className="font-normal text-slate-400">(opsional)</span></span><input disabled placeholder="Pemilihan mata pelajaran belum tersedia" className="w-full rounded-lg border border-slate-200 bg-slate-50 px-3.5 py-3 text-slate-500" /></label>}<EmployeeStatusSelect isStaff={form.role === "staff"} value={form.employment_status_uuid} error={formErrors.employment_status_uuid} onChange={(uuid, name) => { setForm(current => ({ ...current, employment_status_uuid: uuid, employment_status: name })); if (formErrors.employment_status_uuid) setFormErrors(current => ({ ...current, employment_status_uuid: validateWizardField("employment_status_uuid", uuid) })); }} /><DatePicker id="teacher-join-date" label="Tanggal bergabung" required value={form.join_date} error={formErrors.join_date} onChange={value => updateWizardField("join_date", value)} /></div>}
    </StepTransition>
  </div>;

  return <><Helmet><title>Guru dan Staf | Gakuren</title></Helmet><div className="p-4 sm:p-6">{notice && <div role="status" className={`mb-4 rounded-lg border p-4 text-sm ${notice.pending ? "border-amber-200 bg-amber-50 text-amber-800" : "border-blue-200 bg-blue-50 text-blue-800"}`}>{notice.message}</div>}<section className="data-table-card overflow-hidden rounded-xl border border-slate-200 bg-white shadow-card">
